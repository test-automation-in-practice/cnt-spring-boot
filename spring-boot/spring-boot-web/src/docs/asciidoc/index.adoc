...
In order to create a book, the consumer needs to provide a `title` string,
an 13 digit `isbn` and `POST` them to `/api/books`:

include::{snippets}/books/postBook-added/http-request.adoc[]

This will create a new book in the database. The response to this request will
contain the stored data and links to available other resources.

include::{snippets}/books/postBook-added/http-response.adoc[]
