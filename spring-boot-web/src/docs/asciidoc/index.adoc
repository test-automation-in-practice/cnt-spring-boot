...
In order to create a foo, the consumer needs to provide an `bar` string matching
the `Hello .+!` pattern, a `xur` ISO-8601 compliant timestamp and `POST` them to
`/api/foos`:

include::{snippets}/foos/postFoo-created/http-request.adoc[]

This will create a new foo in the database. The response to this request will
contain the stored data and links to available other resources.

include::{snippets}/foos/postFoo-created/http-response.adoc[]
